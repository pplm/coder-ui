...
<Card>
    ...
    <Row>
        ...
        <Col span="12">
            <Card>
                <p slot="title">接口请求属性（已选）</p>
                <Table :columns="columnsListIfc" :data="tableDataIfc" border></Table>
            </Card>
        </Col>
    </Row>
    <Row>
...
export default {
                    ...
                    render: (h, params) => {
                        return h('div', [
                            h('Button', {
                                ...
                                on: {
                                    click: () => {
                                        let attr = this.removeAttr(this.tableDataIfc, params.row.id)[0]
    ...
    methods: {
        ...
        getAttrs () {
            let _self = this;
            util.ajax.get('/ifc/reqAttrs?iid=' + this.iid).then(res => {
                if (res.status === 200) {
                    if (res.data.code === "0") {
